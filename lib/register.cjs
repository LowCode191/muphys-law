...
function readRegister() {
  ...
  lines.forEach((line, index) => {
    ...
    try {
      ...
      // Concurrent syncs racing a stale lock can double-append the same
      // content-derived row (POSIX gives no compare-and-swap on paths, so
      // perfect file-lock exclusion is not winnable — see the sync lock
      // comment). An idempotent reader turns that race's damage into a
      // no-op: correctness lives here, not in the lock. Synthesized legacy
      // ids embed the line index and cannot collide; divergent same-id
      // lines (true corruption) are surfaced by `muphys doctor`.
