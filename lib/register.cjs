...
//   MUPHYS_EMBEDDINGS_URL    e.g. http://localhost:11434/v1/embeddings (Ollama)
//                            or https://api.openai.com/v1/embeddings
//   MUPHYS_EMBEDDINGS_MODEL  e.g. nomic-embed-text / text-embedding-3-small
//   MUPHYS_EMBEDDINGS_API_KEY   optional bearer token
//   MUPHYS_EMBEDDINGS_TIMEOUT_MS   default 4000
...
// Vectors cache to MUPHYS_HOME/embeddings-cache.jsonl keyed by a structural
// hash of [model, text] (no delimiter to inject), so the register is embedded
// once per model, not once per query.
const EMBEDDINGS_URL = process.env.MUPHYS_EMBEDDINGS_URL || "";
const EMBEDDINGS_MODEL = process.env.MUPHYS_EMBEDDINGS_MODEL || "";
const EMBEDDINGS_API_KEY = process.env.MUPHYS_EMBEDDINGS_API_KEY || "";
const EMBEDDINGS_TIMEOUT_MS = Math.max(500, Number(process.env.MUPHYS_EMBEDDINGS_TIMEOUT_MS || 4000) || 4000);
const EMBEDDINGS_CACHE_JSONL = path.join(MUPHYS_HOME, "embeddings-cache.jsonl");
...
async function handleQuery(args, meta = {}) {
  ...
  if (embeddingsEnabled() && query) {
    try {
      ...
      // Exception text can echo the endpoint URL — including userinfo
      // credentials — into a persisted log. Strip every URL before the
      // message is recorded anywhere.
      embeddingsError = String(error?.message || error).replace(/[a-zA-Z][a-zA-Z0-9+.-]*:\/\/[^\s"')]+/g, "<endpoint>").slice(0, 200); // fail-open: lexical ranks stand
    }
  }
  const scored = items
...
function handleApply(args, meta = {}) {
  ...
  // single real event.
  const lessonIds = [...new Set(Array.isArray(args.lessonIds) ? args.lessonIds.filter(Boolean).map(String) : [])].slice(0, 30);
...
// The stdio server loop, exported so the CLI can host it (`muphys mcp`).
